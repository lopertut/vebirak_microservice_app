...
def register(username: str, password: str, email: str):
    ...
    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="This username already used"
        )
...
@app.get("/check_role")
def check_role(user_id: int, role_id: int):
    role_exists = db.check_user_role(user_id, role_id)
